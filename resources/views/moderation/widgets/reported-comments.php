<div class="flex flex-col gap-3">
    ...
    <?php else: ?>
        <?php foreach ($comments as $comment): ?>
            <?php
            $confidence = (int) ($comment['ai_confidence'] ?? $comment['confidence_score'] ?? min(96, 50 + ((int) ($comment['report_count'] ?? 0) * 8)));
            ...
            <article class="rounded-xl border border-white/10 bg-white/5 p-4 transition hover:border-cyan-400/30 hover:bg-cyan-400/5">
                <div class="grid grid-cols-1 gap-4 xl:grid-cols-12">
                    ...
                    <div class="flex items-center xl:col-span-2 xl:justify-end">
                        <a href="/posts/show?id=<?= (int) $comment['post_id']; ?>" class="inline-flex w-full items-center justify-center rounded-lg border border-cyan-400/30 bg-cyan-400/10 px-3 py-2 text-sm font-medium text-cyan-100 transition hover:bg-cyan-400/20 xl:w-auto">Inspect</a>
                    </div>
                </div>
            </article>
        <?php endforeach; ?>
